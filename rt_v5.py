...
CLICK_MODEL_PATH = "gesture_rf.joblib"    
...
has_fired_in_current_lock = False  # 🔥 NEW: The One-Shot Safety Switch
...
def mouse_tick():
    ...
    global overlay
    global has_fired_in_current_lock # 🔥 NEW: Added to globals
    ...
    if mouse_active and collecting and fast_sensor_buf:
        ...
        if current_time <= cursor_freeze_time:
            ...
            # 🔥 NEW: Added "not has_fired_in_current_lock" to prevent double-firing
            if not click_capture_active and not has_fired_in_current_lock and jerk > TAP_JERK_TRIGGER:
                click_capture_active = True
                click_capture_start_time = current_time
                prev_move = np.array([0.0, 0.0])
                cursor_freeze_time = current_time + ML_CAPTURE_WINDOW_MS + 200 
                print("Recording locked gesture data...")
        ...
        if click_capture_active and (current_time - click_capture_start_time >= ML_CAPTURE_WINDOW_MS):
            ...
            if feat is not None:
                ...
                print(f"ML LOCKED PREDICTION: {ml_click_pred}")
                ...
                # elif ml_click_pred == "right_click":
                #     pyautogui.rightClick()
                #     print("Action: RIGHT CLICK")

                elif ml_click_pred == "swipe_down":
                    try: pyautogui.scroll(-SCROLL_STEPS)
                    except: pass
                    print(f"Action: Scrolled DOWN ({-SCROLL_STEPS})")
                    
                elif ml_click_pred == "swipe_up":
                    try: pyautogui.scroll(SCROLL_STEPS)
                    except: pass
                    print(f"Action: Scrolled UP ({SCROLL_STEPS})")
                    
                elif ml_click_pred == "swipe_left":
                    pyautogui.press('right') 
                    print("Action: Next Photo (Right Arrow)")
                    
                elif ml_click_pred == "swipe_right":
                    pyautogui.press('left')  
                    print("Action: Previous Photo (Left Arrow)")

            click_capture_active = False 
            has_fired_in_current_lock = True # 🔥 NEW: Lock it out! No more gestures until you move your mouse again!
        ...
        if feat is not None:
            ...
            if model_says_idle or sensors_say_idle:
                ...
                if idle_time_ms >= IDLE_LOCK_WAIT_MS:
                    ...
                    has_fired_in_current_lock = False # 🔥 NEW: You've returned to Idle, reset the One-Shot lock!
            else:
                idle_time_ms = 0
...
def start_stream(event=None):
    ...
    global has_fired_in_current_lock # 🔥 NEW
    ...
    cursor_freeze_time = 0.0
    click_capture_active = False
    has_fired_in_current_lock = False # 🔥 NEW
    collecting = True
    start_ui_state()
...
overlay.geometry("140x40+20+20")     
overlay.configure(bg="#222222")      

overlay_label = tk.Label(
    overlay, text="🔴 LOCKED", 
    fg="#FF4444", bg="#222222", font=("Segoe UI", 12, "bold")
)
